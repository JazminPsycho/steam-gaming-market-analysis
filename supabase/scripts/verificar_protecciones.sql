...
--
-- 17 pruebas que confirman que RLS, REVOKE DELETE, los triggers de
...
begin
  ...
  begin
    update finanzas.reg_presupuesto set monto_pen = 9999 where id = 'PRE-00002'; v_msg := 'PERMITIÓ editar';
  exception when others then v_msg := 'bloqueado'; end;
  insert into _res values (12,'editar presupuesto congelado','bloqueado',v_msg);

  -- 13 · Un canje se valoriza igual en ambos lados
  begin
    update finanzas.reg_movimientos set monto_original = 3500 where id = 'MOV-2026-000014'; v_msg := 'PERMITIÓ descuadrar';
  exception when others then v_msg := 'bloqueado'; end;
  insert into _res values (13,'descuadrar valorización de un canje','bloqueado',v_msg);

  -- 14 · Los movimientos se clasifican en nivel 2
  begin
  ...
  exception when others then v_msg := 'bloqueado'; end;
  insert into _res values (14,'clasificar movimiento en nivel 1','bloqueado',v_msg);

  -- 15 · Un ingreso no cae en categoría de egreso (FK compuesta)
  begin
    insert into finanzas.reg_movimientos (fecha_prevista, tipo, categoria_id, descripcion, monto_original, metodo_id)
    values ('2026-09-10','ingreso','CAT-E-09-01','[PRUEBA-RLS] tipo cruzado', 10, 'MTD-EFECTIVO'); v_msg := 'PERMITIÓ tipo cruzado';
  exception when others then v_msg := 'bloqueado'; end;
  insert into _res values (15,'ingreso en categoría de egreso','bloqueado',v_msg);

  -- 16 · Un UPDATE que solo mueve el sello no ensucia el log
  ...
  insert into _res values (16,'UPDATE sin cambio real deja log','0 filas nuevas',(v_log_despues - v_log_antes) || ' filas nuevas');

  -- 17 · Un UPDATE real deja una fila con los campos exactos
  select count(*) into v_log_antes from auditoria.log_cambios;
  ...
  select array_to_string(campos_cambiados, ',') into v_msg from auditoria.log_cambios
   where registro_id = 'MOV-2026-000001' and operacion = 'UPDATE' order by id desc limit 1;
  insert into _res values (17,'UPDATE real deja log con campos_cambiados','1 fila · igv',(v_log_despues - v_log_antes) || ' fila · ' || v_msg);
